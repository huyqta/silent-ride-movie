...
export default function MovieCard({ movie, index = 0, showProgress = true }: MovieCardProps) {
    const { isFavorite, addToFavorites, removeFromFavorites, getProgress } = useStore();
    const isLiked = isFavorite(movie.slug);
    const progress = showProgress ? getProgress(movie.slug) : null;
            ...
            <Link href={`/phim/${movie.slug}`} className="block">
                <div className="relative aspect-[2/3] rounded-xl overflow-hidden bg-background-secondary">
                    ...
                    <button
                        ...
                        className={`absolute top-2 right-2 p-1.5 rounded-full transition-all z-10 ${isLiked
